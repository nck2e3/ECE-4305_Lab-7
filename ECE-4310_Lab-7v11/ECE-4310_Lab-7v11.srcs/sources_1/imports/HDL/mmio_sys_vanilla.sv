...
(
   ...
   //slot 4: blinkled 
   chu_blinker_core blinkled_slot4 
   (
    .clk(clk),
    .reset(reset),
    .cs(cs_array[`S4_BLINKER_CORE]),
    .read(mem_rd_array[`S4_BLINKER_CORE]),
    .write(mem_wr_array[`S4_BLINKER_CORE]),
    .addr(reg_addr_array[`S4_BLINKER_CORE]),
    .rd_data(rd_data_array[`S4_BLINKER_CORE]),
    .wr_data(wr_data_array[`S4_BLINKER_CORE]),
    .dout(led[3:0])
    );
